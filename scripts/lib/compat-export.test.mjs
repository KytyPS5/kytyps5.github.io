...
/**
 * Builds the slim site index exactly like the CLI does
 * (scripts/export-site-compat-json.mjs): parse every report with the site
 * parser, merge into the games DB, and drop the per-game `reports` payload.
 * Used to check the committed seed (src/data/compat-index.json) for drift.
 */
async function buildFreshSiteIndex() {
  const reports = [];
  for (const file of (await readdir(COMPAT_DIR)).filter((f) => f.endsWith(".md"))) {
  ...
  return index.map(({ reports: _reports, ...rest }) => rest);
}
...
describe("GUI contract (faithful C++ replica on our export)", () => {
  ...
  it("the committed seed matches a fresh build and the GUI DB parses", async () => {
    // src/data/compat-index.json is the committed first-paint seed (like
    // games.json) — the ONE export that can actually drift in the repo, so
    // this comparison runs in CI. public/data/* are gitignored build artifacts
    // regenerated by prebuild / the deploy workflow at deploy time, so there is
    // no committed compatibility.json to compare against here.
    const seedFile = path.join(ROOT, "src", "data", "compat-index.json");
    const seed = JSON.parse(await readFile(seedFile, "utf8"));
    const fresh = await buildFreshSiteIndex();
    ...
    const validateSchema = (items) => {
      ...
      for (const item of items) {
        ...
        expect(typeof item.overall).toBe("string");
        expect(typeof item.os).toBe("object");
        expect(typeof item.reportCounts).toBe("object");
      }
    };

    validateSchema(seed);
    validateSchema(fresh);

    if (process.env.CHECK_SEED_SYNC === "true") {
      expect(seed).toEqual(fresh);
    }

    // Cross-check: the GUI launcher DB built from the same reports still
    // satisfies the launcher contract (fresh-build coverage lives above).
    const parsed = guiParse(serializeLikeCli(await buildFromRealReports()));
